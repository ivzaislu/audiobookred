...
`/api/v1/search/page` и `/api/v1/search` сначала выполняют обычную
индексируемую выборку ограниченного числа source rows. Затем небольшой набор
дедуплицируется в памяти: первым ключом служит нормализованный `infoHash`, а для
старых строк без hash используется `release:{id}`. Глобальные `GROUP BY`,
`COUNT(DISTINCT infoHash)` и оконное ранжирование из горячего запроса удалены.

Поле `hasMore=true` означает, что быстрый candidate window заполнен и в каталоге
могут быть дополнительные результаты. В таком ответе `total` является нижней
границей уже просмотренных уникальных групп, а не дорогим точным подсчётом всей
выборки. При `hasMore=false` значение `total` точное для найденного набора.

Одинаковые раздачи объединяются после поиска. Сиды и личи складываются по
источникам, `sources` сохраняет значения каждого сайта, а итоговый magnet
объединяет уникальные tracker announce URL. Браузер загружает
`/api/v1/search/facets` отдельно; facets кешируются на 5 минут. Совместимый
`/api/v1/search` не ждёт холодный facet-пересчёт и возвращает cached facets либо
пустые списки.
